...
const AllPostsWithData = compose(
    graphql(AllPostsQuery, {
        ...
        props: (props) => ({
            posts: props.data.allPosts && props.data.allPosts.posts,
        })
    }),
    ...
    props: (props) => ({
        onAdd: post => props.mutate({
            variables: post,
            optimisticResponse: () => ({ putPost: { ...post, __typename: 'Post', version: 1 } }),
        })
    }),
    options: {
        refetchQueries: [{ query: AllPostsQuery }],
        update: (dataProxy, { data: { putPost } }) => {
            const query = AllPostsQuery;
            const data = dataProxy.readQuery({ query });

            data.allPost.posts.push(putPost);
